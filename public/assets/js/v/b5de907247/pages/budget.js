...
async function run() {
  ...
  try {
    ...
    // 取得結果はいったんこの実行だけのローカル変数で受ける。
    // 有効な実行だと確認できるまで共有状態(poolCache/poolKey)には書かない。
    let pool = poolCacheKey === poolKey ? poolCache : [];
    if (pool.length === 0) {
      pool = await provider.searchByBudget({ budget: snap.budget, category: snap.category, limit: 120 });
    }
    // 古い応答は捨てる(条件変更で無効化された場合も含む)。
    // 結果・共有・履歴だけでなく、候補キャッシュも古い実行では書き換えない。
    if (seq !== runSeq) return;
    // キャッシュキーと商品一覧は必ず同じ条件の組で更新する(片方だけ進んだ状態を作らない)
    poolCache = pool;
    poolKey = poolCacheKey;
    const count = snap.count;
    const set = generateBudgetSet(pool, snap.budget, { maxItems: BUDGET_MAX_ITEMS, count, pinned: snap.pinned });
    if (seq !== runSeq) return;
    els.countNote.hidden = true;
    els.countNote.textContent = "";
    ...
    console.error(e);
    // 古い実行の失敗で、新しい実行の結果・共有・案内を壊さない。
    // 表示するのは「いま画面に出ている実行」のエラーだけ。
    if (seq !== runSeq) return;
    els.summary.textContent = "取得に失敗しました。時間をおいて再度お試しください。";
    xShare.hide(); // 失敗した実行の画面で、前回の共有内容を有効にしない
  } finally {
    if (seq === runSeq) {
      busy = false;
      activeConditionKey = "";
